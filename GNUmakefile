.PHONY: all all-pre-hook-local prerequisites rebuild generate-all-plt \
...
#
$(VERSION_FILE):
	@echo "   Generating header file collecting all package versions"          \
...
prepare-release: real-clean clean-release
	...
	-o -name '*.png' -o -name 'erl_crash.dump' \) -exec /bin/rm -f '{}' ';' 2>/dev/null || true
...
# In the current process we do not use install.
install: all
	@echo "   Installing all in "`basename $(PWD)`
	@for m in $(MODULES_DIRS); do if ! ( if [ -d $$m ] ; then cd $$m && \
	$(MAKE) -s install INSTALLATION_PREFIX="$(INSTALLATION_PREFIX)" &&  \
...
stats:
	@$(COMMON_TOP)/src/scripts/make-code-stats.sh `pwd`
