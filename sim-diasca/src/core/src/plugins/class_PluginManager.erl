...
%
% - plugin_table :: table:table( basic_utils:module_name(), term() ] is an
% associative table whose keys are the module name of each plugin (as an atom)
% and whose values are any state information returned by a given plugin
...
construct( State, ?wooper_construct_parameters ) ->
	...
	InitState = setAttribute( TraceState, trace_categorization,
...
notifySimulatorStart( State ) ->
	...
	{ FinalChanges, FinalTable } = lists:foldl(
		fun( { Mod, PlugState }, { Changes, Table } ) ->
				...
				% Update state:
				NewTable = table:addEntry( _K=Mod, _V=NewPlugState, Table ),
...
% (helper)
%
create_initial_plugin_table( Plugins, _State ) ->
	...
	case length( Plugins ) of
		...
		_L ->
			...
			lists:foldl( fun( Mod, Table ) ->
							...
							table:addEntry( _K=Mod, _V='undefined', Table )
						 end,
						 _Acc0=EmptyTable,
						 _List=Modules )
...
notify_event( Event, State ) ->
	...
	NewTable = lists:foldl( fun( { Mod, PlugState }, Table ) ->
					...
					% Update state:
					table:addEntry( _K=Mod, _V=NewPlugState, Table )
