...
% class)
%
% - initial_actors :: [ pid() ] is a list of the PID of all initial actors; it
...
%
%  - with the round-robin policy: list_utils:ring( net_utils:atom_node_name() ),
...
construct( State, ?wooper_construct_parameters ) ->
	...
	{ Seed, _SeedInfoString, OrderingMode } = manage_seeding( SimulationMode ),
	...
	PlacementPolicyData = case PlacementPolicy of
		...
		round_robin ->
			...
			list_utils:list_to_ring( NodeList )
...
select_node_with_round_robin( State ) ->
	...
	{ SelectedNode, NewNodeRing } = list_utils:head( NodeRing ),
...
create_initial_actor( ActorClassName, ActorConstructionParameters, Node,
	...
	ActorPid = apply( ActorClassName, remote_synchronisable_new_link,
...
create_runtime_actor( ActorClassName, ActorConstructionParameters, ActorTag,
	...
	%io:format( "Will create now a runtime actor ~w with parameters ~p "
	%		   "and tag ~w on node ~w.~n",
	...
	%io:format( "## Creation of runtime actor ~p (~p) on node ~s.~n",
	%		   [ ActorPid, ActorClassName, Node ] ),
