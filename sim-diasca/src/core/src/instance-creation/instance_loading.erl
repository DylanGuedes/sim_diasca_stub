...
-define( TraceEmitterCategorization, "Core.LoadBalancing.Instances." ).
-define( loader_cat, ?TraceEmitterCategorization ++ "Loading" ).
-define( reader_cat, ?TraceEmitterCategorization ++ "Creating" ).
...
manage_initialisation( InitialisationFiles, NodeCount, LoadBalancerPid ) ->
	...
	io:format( "Loading initial instances from: ~s",
...
%
-spec initialisation_waiting_loop( pid(), basic_utils:count(), [ pid() ],
		  [ pid() ], time_utils:precise_timestamp(), pid() ) ->
										 basic_utils:void().
% First clause, for termination:
initialisation_waiting_loop( UserIdResolverPid, _CreationCount, Creators,
					_Readers=[], _LoadStartTimestamp, LoadBalancerPid ) ->
...
read_init_file( Filename, Creators, InstanceLoaderPid ) ->
	...
	case file_utils:is_existing_file_or_link( Filename ) of
		...
		false ->
			basic_utils:display_error( "~n  Error, initialisation file '~s' "
									   "could not be found from node '~s'.~n",
									   [ Filename, node() ] ),
	...
	% problem here.
	%
	InstanceCreationCount = read_all_lines( InitFile, CreationChunkCount,
											Creators, Filename ),
...
%
-spec read_all_lines( file_utils:file(), basic_utils:count(), [ pid() ],
					  file_utils:file_name() ) -> basic_utils:count().
read_all_lines( File, ChunkCount, Creators, Filename ) ->
	...
	read_all_lines( File, ChunkCount, _CreationCount=0,
					_RemainingFreeCreators=Creators, _WaitedCreators=[],
					BinFilename, _LineCount=1 ).
...
%
-spec read_chunk( file_utils:file(), basic_utils:count(), line_number() ) ->
...
parse_creation_line( _LineInfo={ LineNumber, BinLine }, BinFilename,
	...
	case text_utils:trim_whitespaces( Line ) of
		...
		L=[ ${ | _T ] ->
			...
			create_instance_from( _CreationClause=L, _IdInfo=none, LineContext,
								  IdResolverPid, LoadBalancerPid );
...
-spec replace_identifiers_by_pid( [ method_argument() ], pid(), pid(),
			   line_context() ) -> { [ method_argument() ], identifier_info() }.
...
transform_argument( Arg, IdResolverPid, LoadBalancerPid, FirstUserId,
	...
	IdTransformer = fun
						...
						( _Term={ user_id, NonStringId }, _CurrentFirstId ) ->
							...
							Filename = text_utils:binary_to_string(
										 BinFilename ),
	...
	meta_utils:traverse_term( _TargetTerm=Arg, _TypeDescription=tuple,
...
% Parameters:
%
% - IdTable :: table:table( user_identifier(), { blank_pid, pid() } | pid() ) is
...
%
-spec user_identifier_resolver_loop( table:table(), pid() ) -> no_return().
...
declare_id( BinId, TargetNode, IdTable, LoadBalancerPid, CallerPid ) ->
	...
	%
	case table:lookupEntry( _Key=BinId, IdTable ) of
		...
		key_not_found ->
			...
			table:addEntry( _K=BinId, _Value=BlankPid, IdTable );
...
resolve_id( BinId, IdTable, LoadBalancerPid, CallerPid ) ->
	...
	case table:lookupEntry( _Key=BinId, IdTable ) of
		...
		key_not_found ->
			...
			table:addEntry( _K=BinId, _Value={ blank_pid, BlankPid }, IdTable );
...
wrap_up_and_terminate( IdTable ) ->
	...
	case table:foldOnEntries( CheckFun, _InitialIdAcc=[], IdTable ) of
...
% Returns a string corresponding to the specifiied initialisation information,
% like:
%
% "my_city" <- { class_City, [ "Paris", "city_pa" ] }.
%
-spec get_instance_initialisation_line( wooper:class_name(),
										wooper:method_arguments(),
...
get_instance_initialisation_line( Classname, Parameters,
	...
	text_utils:format( "\"~s\" <- ", [ UserIdentifier ] )
		++ get_instance_description( Classname, Parameters ).
