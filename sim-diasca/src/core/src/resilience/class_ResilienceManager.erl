...
%
%-type k_map() :: table:table( node_name(), k_record() ).
-type k_map() :: table:table().


% To associate the PID of a resilience agent to its node:
%
%-type node_table() :: table:table( node_name(), pid() ).
-type node_table() :: table:table().
...
% charge of that node
%
% - last_serialisation_timestamp :: time_utils:timestamp() records when the last
% serialisation (if any) was done, in order to know when the next one is to
% happen
%
...
%
% - start_timestamp :: time_utils:timestamp() records a timestamp corresponding
...
perform_rollback( Tick, Diasca, CrashedNodes, State ) ->
	...
	SurvivorAgentPidList = [
			...
			begin
				...
				SurvivorAgentPid = table:getEntry( _K=Node, NodeTable ),
...
dispatch_serialisations( _CrashedNodes=[ N | T ], WeightedNodes, KMap ) ->
	...
	Candidates = ( table:getEntry( _K=N, KMap ) )#k_record.secured_by,
...
register_nodes( _ProtectedNodes=[ N | T ], RingNodes, ResilienceLevel, KMap ) ->
	...
	UpdatedKMap = table:addEntry( _K=N, _V=NewKRecord, SecuredKMap ),
...
add_securing_node( SecuringNode, SecuredNodes, KMap ) ->
	...
	lists:foldl( fun( SecuredNode, AccKMap ) ->
		...
		table:addEntry( _K=SecuredNode, _V=NewKRecord, AccKMap )
...
obtain_k_record_for( Node, KMap ) ->
	...
	case table:lookupEntry( _K=Node, KMap ) of
...
create_node_table( AllComputingNodes, KMap, ResilienceDirBin ) ->
	...
	lists:foldl( fun( Node, AccTable ) ->
		...
		NodeKRecord = table:getEntry( _K=Node, KMap ),
		...
		table:addEntry( _Key=Node, _Value=AgentPid, AccTable )
...
update_node_table( SurvivingNodes, PreviousNodeTable, KMap ) ->
	...
	lists:foldl( fun( Node, AccTable ) ->
		...
		AgentPid = table:getEntry( _K=Node, PreviousNodeTable ),
...
trigger_serialisation( Tick, Diasca, State ) ->
	...
	NodeAgents = [ AgentPid || { _Node, AgentPid } <- table:enumerate(
													   ?getAttr(node_table) ) ],
