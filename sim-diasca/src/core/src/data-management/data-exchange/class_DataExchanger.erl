...
-export([ executeRequestInTreeHelper/3,
...
% TO-DO: trigger appropriately table:optimise/1 calls on data_table
...
% exchangers
%
% - data_table :: table:table( key(), qualified_value() ), i.e. is a table whose
% keys are the ones of the data entry, and whose values are { Value, Qualifier }
% pairs, where Value is the data associated to the key and Qualifier is either
% 'const' or 'mutable'
%
...
construct( State, ExchangerName, { ConfigurationFileList, RootTimeManagerPid } )
  when is_list( ConfigurationFileList ) andalso is_pid( RootTimeManagerPid ) ->
	...
	{ _Message, ReadState } = case ConfigurationFileList of
...
%	?send_debug_fmt( NewState, "Data table after construction: ~s.",
		%	  [ table:toString( getAttribute( NewState, data_table ) ) ] ),
...
readInitialData( State, Keys ) when is_list( Keys ) ->
	...
	ReadPairs = try table:selectEntries( Keys, ?getAttr(data_table) )
...
readInitialData( State, Key ) when is_atom( Key ) ->
	...
	case table:lookupEntry( Key, ?getAttr(data_table) ) of
...
readData( State, Keys ) when is_list( Keys ) ->
	...
	ReadPairs = try
		...
		table:selectEntries( Keys, ?getAttr(data_table) )
...
defineDataHelper( State, _Entry={ Key, V } ) ->
	...
	% without further ado:
	NewTable = table:addEntry( Key, V, ?getAttr(data_table) ),
...
defineDataHelper( State, EntryList ) -> % when is_list(EntryList) ->
	...
	NewTable = table:addEntries( EntryList, ?getAttr(data_table) ),
...
getAllData( State ) ->
	?wooper_return_state_result( State,
								 table:enumerate( ?getAttr(data_table) ) ).
...
get_entry_to_define( _Entry={ K, V }, Table ) when is_atom( K ) ->
	...
	case table:lookupEntry( K, Table ) of
...
parse_files( FileList, NodeType, State ) ->
	...
	%table:display( "Parsed data table is", NewTable ),

	% Optimising the load factor helps a lot the table look-up
	% performances:
	%
	setAttribute( State, data_table, table:optimise( NewTable ) ).
			...
			case file:consult( Filename ) of
				...
				{ ok, EntryList } ->
					...
					try
						...
						table:addEntries( DefinitionList, Table )
...
executeRequestInTreeHelper( State, MethodName, Parameters ) ->
	...
	{ ExecutedState, LocalRes } = executeRequest( State, MethodName,
												  Parameters ),
