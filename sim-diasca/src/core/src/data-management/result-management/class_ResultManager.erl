...
% possibly post-process (ex: generating appropriate rendering) to the user the
...
% The basic probes that are tracked will be deleted by the result managers. The
% ones that are not tracked are to be managed by their creator.
...
%
% - basic_probe_table is a table whose keys are names of basic probe names (as
% binaries) and whose values are basic_probe_entry() records
%
% - basic_probe_default_options is a list of default options for basic
% probes
%
% - virtual_probe_table is a table whose keys are names of virtual probes (as
% binaries) and whose values are virtual_probe_entry() records
%
% - datalogger_default_options is a list of default options for virtual probes
%
% - pid_to_queue :: table( pid(), result_queue#id() ) is a table whose keys are
% the PID of the tracked producers and whose associated values are the ID of the
% result queue in charge of a given producer: allows to convert efficiently a
% producer PID into a queue ID
%
...
construct( State, ?wooper_construct_parameters ) ->
	...
	FinalState = setAttributes( TraceState, [
		...
		{ basic_probe_table, table:new() },
		{ basic_probe_default_options, [ data_and_plot ] },
		{ virtual_probe_table, table:new() },
		{ datalogger_default_options, [ plot_only ] },
		{ pid_to_queue, table:new() },
...
destruct( State ) ->
	...
	% Deleting all basic probes that are tracked (others are to be managed
	% explicitly by their user):
	%
...
declareProbe( State, BinProbeName, IsToBeTracked ) ->
	...
	case is_result_wanted( BinProbeName, basic_probe, State ) of
		...
		{ true, ProbeOptions } ->
			...
			case table:hasEntry( _Key=BinProbeName, ProbeTable ) of
...
register_probe( BinProbeName, ProbeOptions, IsToBeTracked, ProbePid, State ) ->
	...
	NewProbeTable = table:addEntry( _K=BinProbeName, _V=ProbeEntry,
	...
	case IsToBeTracked of
		...
		true ->
			...
			NewPidToQueueTable = table:addEntry( _Key=ProbePid,
...
notifyResilienceAgentsOfProbes( State, NodeAgents ) ->
	...
	EmptyNodeTable = lists:foldl( fun( AgentPid, TableAcc ) ->
				table:addEntry( _K=node( AgentPid ),
	...
	FilledNodeTable = lists:foldl( fun( ProbePid, TableAcc ) ->
						...
						{ AgentPid, ProbeList } = table:getEntry( NodeKey,
																  TableAcc ),
						table:addEntry( NodeKey,
...
simulation_started( State ) ->
	...
	_BasicProbeString = text_utils:binary_list_to_string( BasicProbeBinNames ),
	%_BasicProbeCount = length( BasicProbeBinNames ),

	VirtualProbeTable = ?getAttr(virtual_probe_table),
	OptimisedVirtualProbeTable = table:optimise( VirtualProbeTable ),
	%table:display( "Virtual probe table",
	%     OptimisedVirtualProbeTable ),

	VirtualProbeBinNames = table:keys( OptimisedVirtualProbeTable ),
	_VirtualProbeString = text_utils:binary_list_to_string(
						   VirtualProbeBinNames ),
	%_VirtualProbeCount = length( VirtualProbeBinNames ),
...
simulation_succeeded( State ) ->
	...
	case file_utils:is_existing_directory( ResultDirName ) of
		...
		true ->
			...
			?error_fmt( "Result directory ('~s') is already existing, "
						"which is both unlikely and abnormal.",
						[ ResultDirName ] ),

			throw( { already_existing_result_directory, ResultDirName } )
...
trigger_virtual_probe_results( State ) ->
	...
	case ?getAttr(result_spec) of
		...
		_Other ->
			...
			NewPidToQueueTable = table:addEntry( _K=DataloggerPid,
...
load_result_queues( _Queues=[ Q | T ], PidToQueueTable, ProbeTable,
	...
	UpdatedPidToQueueTable = table:addEntries( NewPidEntries,
											   PidToQueueTable ),
...
trigger_producer( ProducerName, ProbeTable ) ->
	...
	ProbeEntry = table:getEntry( ProducerName, ProbeTable ),
...
wait_and_exhaust_queues( Queues, PidToQueueTable, TotalProducerCount,
	...
	ActualProducerPid = receive
		...
		{ wooper_result, { ProducerPid, archive, BinArchive } } ->
			...
			file_utils:zipped_term_to_unzipped_files( BinArchive,
												  ?getAttr(result_dir) ),
			ProducerPid;
...
update_queues_after_result( ProducerPid, PidToQueueTable, ProbeTable,
	...
	QueueId = table:getEntry( ProducerPid, PidToQueueTable ),

	{ Queue, OtherQueues } = extract_queue_by_id( QueueId, Queues ),

	NewPidToQueueTable = table:removeEntry( ProducerPid, PidToQueueTable ),
				...
				simulation_succeeded ->
					...
					receive
						...
						{ wooper_result, results_browsed } ->
							io:format( "Result reports have been "
