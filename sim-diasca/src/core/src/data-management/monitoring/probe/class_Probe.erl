...
%
% - sample_count :: basic_utils:count() keeps track of the number of samples
...
% this probe was fed with; it is not a table because gnuplot may prefer that the
% rows be ordered, and immediate writing might be requested. It is an ordered
% list (in reverse chronological order, as new samples are added at the head)
% which contains { TickOffset, Samples } entries, with Samples being a tuple
% whose size can increase over time, if updateCurveInformation/2 is called
...
%
% - data_file :: basic_utils:maybe( file_utils:file() ) is the file object (if
...
construct( State, { Name, ProbeOptions }, CurveNames, Zones, Title,
	...
	% Finally waits the answer from the result manager to the declareProbe/2
	% call above:
	%
	...
	receive
		...
		%			 "The probe ~s would not produce an expected result.",
		%			 [ Name ] ),
		...
		DataFile ->
			...
			case ?getAttr(deferred_data_writes) of
				...
				false ->
					...
					% We were performing immediate writes here, and due to the
					...
					case file:close( DataFile ) of
						...
						{ error, Reason } ->
							?error_fmt( "Error while closing probe '~s': ~p.",
										[ ?getAttr(name), Reason ] ),
							file:close( DataFile ) ;
...
setData( State, Tick, Samples ) ->
	...
	case ?getAttr(deferred_data_writes) of
		...
		true ->
			...
			?wooper_return_state_only( appendToAttribute( TrackedState,
					data_table, { RecordedTick, Samples } ) );

		false ->
			% Lower-level I/O will attempt a bit of deferred write nevertheless:
			write_row( ?getAttr(data_file), RecordedTick, Samples ),
			?wooper_return_state_only( TrackedState )
...
sendResults( State, [ plot_only ] ) ->
	...
	case ?getAttr(sample_count) of
		...
		0 ->
		%	?warning_fmt( "Probe '~s' did not receive any data sample, hence "
		%				  "does cannot yield a plot.", [ Name ] ),
			...
			?wooper_return_state_result( FinalState, Result );
		...
		%				[ ReportFilename ] ),
			...
			DataFilename = text_utils:binary_to_string(
...
%
-spec write_header( file_utils:file(), curve_entries(), zone_entries(),
					probe_settings(), string(),
					class_ResultManager:meta_data() ) -> basic_utils:void().
...
%
-spec write_row( file_utils:file(), probe_tick(), sample_data() ) ->
					   basic_utils:void().
write_row( File, Tick, DataTuple ) ->
	file_utils:write( File, format_row( Tick, DataTuple ) ).
...
get_x_range_option( Settings ) ->
	...
	case Settings#probe_settings.x_range of
			...
			{ MinX, MaxX } when is_integer( MinX ) andalso
								is_integer( MaxX ) ->
...
generate_report( Name, State ) ->
	...
	% absolute path for the PNG is not an option either, as we move the files to
	% the result directory
	%
	Command = executable_utils:get_gnuplot_path() ++  " '" ++ CommandFilename
		++ "'",

	OutputMessage = case system_utils:run_executable( Command,
		...
		%	?warning_fmt( "Report generation succeeded for '~s', but output "
		%				  "following information: ~s", [ Name, CmdOutput ] ),
		...
		{ ReturnCode, CmdOutput } ->
			...
			?error_fmt( "Report generation failed for '~s' (code: ~B), "
						"following information reported: ~s",
		...
		false->
			case OutputMessage of
				...
				[] ->
					...
					?error_fmt( "Report generation failed for '~s' "
								"(result file '~s' not available, no "
				...
				_ ->
					?error_fmt( "Report generation failed for '~s' "
								"(result file '~s' not available, message "
								"output: '~s')",
