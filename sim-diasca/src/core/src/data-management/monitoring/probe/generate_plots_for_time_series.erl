...
select_data_files( DirectoryName ) ->
	...
	{ RegularFiles, _Directories, _OtherFiles, _Devices } =
...
manage_workers( _DataFilenames=[], _CurrentWorkers=[], GeneratedFilenames,
	...
	io:format( "All ~B reports were successfully generated: ~s~n",
			  [ length( GeneratedFilenames ),
				text_utils:string_list_to_string( GeneratedFilenames ) ] );
...
manage_workers( _DataFilenames=[], _CurrentWorkers=[], GeneratedFilenames,
	...
	io:format( "Out of ~B reports, ~B were successfully generated:~s~n"
			  ...
			  [ GenLen + ErrLen, GenLen,
				text_utils:string_list_to_string( GeneratedFilenames ), ErrLen,
				text_utils:string_list_to_string( ErrorStrings ) ] );
...
manage_plot( DataFilename, DispatcherPid ) ->
	...
	CommandFilename = file_utils:replace_extension( DataFilename, ".dat",
													".p" ),
...
generate_report( DataFilename, CommandFilename ) ->
	...
	Succeeded = case system_utils:run_executable( Command ) of
		...
		{ _ReturnCode=0, CmdOutput } ->
			...
			io:format( "[warning] Report generation succeeded for ~s, "
		...
		{ ReturnCode, CmdOutput } ->
			% Would be ?error_fmt:
			io:format( "[error ~B] Report generated failed for ~s and output "
	...
	case Succeeded of
		...
		true ->
			...
			case file_utils:is_existing_file( TargetFilename ) of
				...
				false->
					{ failure, io_lib:format( "no generated file for ~s",
											  [ DataFilename ] ) }
