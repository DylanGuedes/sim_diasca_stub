...
-record( deployment_settings, {
	...
	% available (installed) on the target platform (ex: on the user node).
	%
	% So they will not be deployed, yet the engine must add their directories to
	% its code path so that these prerequisites can be used.
	%
	...
	%    port; note that this port must match in all cases the Common one (see
	%    the EPMD_PORT variable in common/GNUmakevars.inc for its defintion and
