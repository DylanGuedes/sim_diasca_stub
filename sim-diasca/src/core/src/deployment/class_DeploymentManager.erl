...
-define( wooper_method_export, onHostDeploymentFailure/3,
		 getLoadBalancer/1, getRootTimeManager/1, getResultManager/1,
		 getComputingNodes/1,
...
construct( State,
	...
	%
	_TickChange = net_kernel:set_net_ticktime( InterNodeSeconds ),
	...
	VersionStrings = { _CommonVersionString, _WOOPERVersionString,
		_TracesVersionString, _SimDiascaVersionString } = get_version_string(),
	...
	code_utils:declare_beam_directories(
	  DeploymentSettings#deployment_settings.additional_beam_directories ),
...
wait_setup_outcome( Waited, Available, Failed, CollectTimeOut,
	...
	receive
		...
		{ onHostDeploymentFailure, [ HostManagerPid, Reason ] } ->
			...
			#computing_host_info{ host_name=_Hostname,
								  user_name=_Username,
								  node_name=_Nodename } =
...
activateDatabase( State ) ->
	...
	case ?getAttr(database_running) of
		...
		false ->
			...
			mnesia:delete_schema( DatabaseEnabledNodes ), 
...
% (helper)
%
get_deploy_time_out( DeploymentSettings, _State ) ->
...
set_up_computing_nodes( BaseNodeName, HostUserList, InterNodeSeconds, State ) ->
	...
	HostInfos = [ #computing_host_info{
		...
		user_name=text_utils:string_to_binary( U ),

		node_name=net_utils:get_fully_qualified_node_name( BaseNodeName, H,
														   NamingMode ),
...
rebuild_directory( Directory, State ) ->
	...
	case system_utils:run_executable( Command ) of
		...
		{ _ReturnCode=0, _CmdOutput=[] } ->
			io:format( "Directory ~s successfully rebuilt.", [ Directory ] );

		{ _ReturnCode=0, CmdOutput } ->
			io:format( "Directory ~s successfully rebuilt, yet a message "
