...
construct( State, SimulationTickDuration, InteractivityMode,
	...
	{ ParentManager, _Description } = case ParentManagerInformation of
	...
	StartState = setAttributes( CategorizedState, [
		...
		% TODO: uncommento to force interactive mode
...
destruct( State ) ->
	...
	%eprof:stop_profiling(),
  	%eprof:log("/home/eduardo/teste_eprof"),
    	%eprof:analyze(),
...
notifyTriggeredWatchdogCompleted( State, _WatchdogTickOffset, _WatchdogDiasca ) ->
   ...
   interactive = ?getAttr(interactivity_mode),
	...
	case ?getAttr(waited_count) of
		...
		NonZeroCount ->
			...
			io:format( "timerTickFinished trigger: "
...
onWooperExitReceived( State, _Pid, _ExitType ) ->
...
init( State ) ->
	...
	_StopString = case ?getAttr(stop_tick_offset) of
...
simulationStarted( State, SimulationInitialTick,
	...
	%
	_ActorCount = start_actors_by_chunks( InitialActors, StartMessage ),
...
% (helper)
%
manage_suspension( _CurrentTickOffset, State ) ->
...
%check_diasca_triggered( TerminatingActors, ScheduledActors ) ->
	...
	% fase = list_utils:has_duplicates( ?list_impl:to_list( ScheduledActors ) ).
...
% (helper)
%
check_not_in_slot( _ActorPid, { _TickOffset, _ActorList } ) ->
