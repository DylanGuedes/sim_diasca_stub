...
%
% - trackers :: table:table( net_utils:atom_node_name(), pid() ) allows to tell,
% for a given node, what is the PID of its local instance tracker
%
% - troubleshooting_mode :: boolean() tells whether the troubleshooting mode is
% activated
%
% - actor_table :: table:table( pid(), actor_info() ) is a table which allows to
% obtain from a PID the information kept about the corresponding (supposedly
% still living) local actor
%
% - zombi_table :: table:table( pid(), actor_info() ) is a table, only filled in
% troubleshooting mode, which allows to keep the information of actors that were
% deleted, to better diagnose issues with life-cycle management (on deletion,
% the instance information goes from actor_table to this zombi_table)
%
% - agent_table :: table:table( pid(), agent_ref() ) is a table which allows to
% obtain from a PID the corresponding reference of the simulation service agent
%
% - producer_table :: table:table( pid(), producer_ref() ) is a table which
% allows to convert a PID into the reference of a result producer
%
...
% - aai_to_pid :: table:table( class_Actor:aai(), pid() ) is an (optional)
% reverse table, which allows to translate any AAI into a PID
%
% - agent_ref_to_pid :: table:table( agent_ref(), pid() ) is an (optional)
% reverse table, which allows to translate any AAI into a PID
%
% - producer_ref_to_pid :: table:table( producer_ref(), pid() ) is an (optional)
% reverse table, which allows to translate any reference of a result producer
% into a PID
%
% - crashed_node_table :: table:table( net_utils:atom_node_name(),
% net_utils:atom_node_name() ) allows to convert the name of a crashed node into
% the name of the node that is taking it in charge after a rollback
%
...
construct( State, ?wooper_construct_parameters ) ->
	...
	{ _ParentString, ParentPid } = case ParentTrackerPid of
	...
	StartingState = setAttributes( TraceState, [
		...
		{ actor_table, table:new() },
		{ zombi_table, table:new() },
		{ agent_table, table:new() },
		{ producer_table, table:new() },
		{ pid_resolution_enabled, true },
		{ aai_to_pid, table:new() },
		{ agent_ref_to_pid, table:new() },
		{ producer_ref_to_pid, table:new() },
...
declareTrackers( State, Trackers ) ->
	...
	TrackerTable = lists:foldl(
				 ...
				 fun( TrackerPid, AccTable ) ->
					...
					table:addEntry( _K=TrackerNode, _V=TrackerPid, AccTable )
				 end,
	...
	?wooper_return_state_result( setAttribute( State, trackers, TrackerTable ),
								 trackers_declared ).
...
registerActor( State, ActorPid, ActorInfo ) ->
	...
	NewActorTable = table:addEntry( _K=ActorPid, _V=ActorInfo, Table ),
	...
	ReverseState = case ?getAttr(pid_resolution_enabled) of
			...
			true ->
				...
				% table types):
				ReverseTable = table:addEntry( _Key=ActorInfo#actor_info.aai,
											   _Value=ActorPid,
...
unregisterActor( State, ActorPid, ActorClassname ) ->
	...
	case table:lookupEntry( ActorPid, ActorTable ) of
		...
		{ value, ActorInfo } ->
			...
			ReverseState = case ?getAttr(pid_resolution_enabled) of
					...
					true ->
						...
						ReverseTable = table:removeEntry( AAI,
													?getAttr(aai_to_pid) ),
						setAttribute( State, aai_to_pid, ReverseTable );
			...
			NewActorTable = table:removeEntry( ActorPid, ActorTable ),
			...
			NewZombiTable = case ?getAttr(troubleshooting_mode) of
				...
				true ->
					table:addEntry( _K=ActorPid, _V=ActorInfo, ZombiTable );
...
unregisterAgent( State, AgentPid ) ->
	...
	ReverseState = case ?getAttr(pid_resolution_enabled) of
		...
		true ->
			...
			AgentRef = table:getEntry( AgentPid, AgentTable ),
			ReverseTable = table:removeEntry( AgentRef,
	...
	NewAgentTable = table:removeEntry( AgentPid, AgentTable ),
...
registerResultProducer( State, ProducerRef ) ->
	...
	ReverseState = case ?getAttr(pid_resolution_enabled) of
			...
			true ->
				NewReverseTable = table:addEntry( ProducerRef, ProducerPid,
	...
	NewProducerTable = table:addEntry( _K=ProducerPid, _V=ProducerRef,
...
unregisterResultProducer( State, ProducerPid ) ->
	...
	ReverseState = case ?getAttr(pid_resolution_enabled) of
		...
		true ->
			% Find first the reference to remove:
			ProducerRef = table:getEntry( ProducerPid, ProducerTable ),
			ReverseTable = table:removeEntry( ProducerRef,
											  ?getAttr(producer_ref_to_pid) ),
	...
	NewProducerTable = table:removeEntry( ProducerPid, ProducerTable ),
...
resolvePid( State, PidToResolve, CallerPid ) ->
	...
	case node( PidToResolve ) of
		...
		RemoteNode ->
			...
			%
			RemoteTrackerPid = table:getEntry( RemoteNode, ?getAttr(trackers) ),
...
%
-spec setCrashedNodeMapping( wooper:state(), table:table() )
...
%
-spec actor_table_to_string( table:table() ) -> string().
...
%
-spec actor_info_to_string( actor_info(), pid() ) -> string().
...
%
-spec agent_table_to_string( table:table() ) -> string().
...
%
-spec producer_table_to_string( table:table() ) -> string().
...
get_local_actor_info( ActorPid, State ) ->
	...
	case table:lookupEntry( _Key=ActorPid, ActorTable ) of
		...
		key_not_found ->
			...
			case ?getAttr(troubleshooting_mode) of
				...
				true ->
					...
					ActorEntry = table:getEntry( _K=ActorPid,
												 ?getAttr(zombi_table) ),
...
resolve_locally( Pid, State ) ->
	...
	case table:lookupEntry( _Key=Pid, ActorTable ) of
		...
		key_not_found ->
			...
			case table:lookupEntry( _Key=Pid, AgentTable ) of
				...
				key_not_found ->
					...
					case table:lookupEntry( _Key=Pid, ProducerTable ) of
...
register_agent_helper( AgentClassName, AgentPid, State ) ->
	...
	ReverseState = case ?getAttr(pid_resolution_enabled) of
			...
			true ->
				NewReverseTable = table:addEntry( AgentRef,	AgentPid,
												  ?getAttr(agent_ref_to_pid) ),
	...
	NewAgentTable = table:addEntry( _K=AgentPid, _V=AgentRef,
									?getAttr(agent_table) ),
