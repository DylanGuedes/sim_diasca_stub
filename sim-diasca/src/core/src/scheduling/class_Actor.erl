...
-export_type([ aai/0, name/0, internal_name/0,
			   ...
			   actor_oneway_return/0, actor_pid/0, actor_message/0, tag/0,
...
onFirstDiasca( State, _SendingActorPid ) ->
	...
	{ _IgnoredState, _ActualClassname } = executeRequest( State, getClassName ),
...
% (actor oneway)
%
-spec onActorCreated( wooper:state(), pid(), tag(), pid() ) -> oneway_return().
...
execute_reordered_oneways( _Messages=[
	   { SenderPid, _SenderAAI, { OnewayName, OnewayArgList } }
									  | MessageTuples ],
...
-spec create_actor( class_name(), [ method_argument() ], tag(),
					wooper:state() ) -> wooper:state().
create_actor( ActorClassname, ActorConstructionParameters, ActorTag,
			  State ) when is_atom( ActorClassname ) andalso
						   is_list( ActorConstructionParameters ) ->
...
create_placed_actor( ActorClassname, ActorConstructionParameters, ActorTag,
  ...
  when is_atom( ActorClassname ) andalso
	...
	send_actor_message( ?getAttr(load_balancer_pid),
		{ createRuntimePlacedActor, [ ActorClassname,
					 ActorConstructionParameters, ActorTag, PlacementHint ] },
						State ).
