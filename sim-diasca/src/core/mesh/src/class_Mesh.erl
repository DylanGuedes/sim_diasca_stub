...
construct( State, { Name, OutputDirectory }, MeshOptions ) ->
	...
	%io:format( "## Mesh dir: ~s~n", [ OutputDirectory ] ),
...
getMeshInformation( State ) ->
	...
	?wooper_return_state_result( State, text_utils:format(
...
% Sets the directory in which this mesh will be rendered.
%
% (oneway)
%
-spec setRenderingDirectory( wooper:state(), file_utils:directory_name() ) ->
								oneway_return().
...
% Copies the probe rendering to specified directory and displays it.
%
...
% (request)
%
-spec displayRenderingIn( wooper:state(), file_utils:directory_name() ) ->
						request_return( 'rendering_displayed' ).
displayRenderingIn( State, TargetDirectoryName ) ->
	...
	?wooper_return_state_result( State, rendering_displayed ).
...
% Generates a topological view for this mesh.
%
% (helper)
%
...
generate_topological_view( DisplayWanted, BaseFileName, State ) ->
	...
	%?debug_fmt( "Generating topology for '~s': graph in '~s', view in '~s', "
		%		"while in '~s'.~n",
...
init_common( Options, State ) ->
	...
	GraphLabel = io_lib:format( "Topological view of mesh '~s'", [ Name ] ),
