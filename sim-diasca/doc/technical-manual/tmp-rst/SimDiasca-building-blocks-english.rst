...
The only drawbacks of this trace mode are that:
 ...
 - it requires a specific tool (LogMX), properly obtained, installed and configured with the Sim-Diasca parser
...
As a result, simulation traces output in the corresponding trace file (ex: ``soda_deterministic_integration_test.traces``) will be in plain text and encoded for human readability (within a nice ASCII-art array); they can thus be then read thanks to any text viewer.
...
Then, as soon as the simulation has stopped, traces will be properly aggregated and formatted from Sim-Diasca, a PDF being generated and then automatically displayed. This PDF can be useful to share simulation results asynchronously (ex: with remote colleagues).
