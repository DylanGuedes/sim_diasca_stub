...
- helper functions shall be identified as such, with an ``(helper)`` comment
...
For example, as of May 2015, we have for the Sim-Diasca stack (i.e. from ``common`` to ``sim-diasca``):

- 278 source files (``*.erl``), 76 header files (``*.hrl``)
- a grand total of 151708 lines:
 - 49607 of which (32.6%) are blank lines
 - 47503 of which (31.3%) are comments
 -  of which (%) are code
